#include "main.h"

/**
 * is_all_printable - Evaluates if a character is printable
 * @c: Character to be evaluated.
 *
 * Return: 1 if c is printable, 0 otherwise
 */
int is_all_printable(char c)
{
	if (c >= 32 && c < 127)
		return (1);

	return (0);
}

/**
 * append_hex_code - Append a ascci in hexadecimal code to buffer
 * @buffer: Array of characters.
 * @i: Index to start appending.
 * @ascii_code: ASSCI CODE.
 * Return: Always 3
 */
int append_hex_code(char ascii_code, char buffer[], int i)
{
	char map_to[] = "0123456789ABCDEF";
	/* The hexa format code is always 2 digits long */
	if (ascii_code < 0)
		ascii_code *= -1;

	buffer[i++] = '\\';
	buffer[i++] = 'x';

	buffer[i++] = map_to[ascii_code / 16];
	buffer[i] = map_to[ascii_code % 16];

	return (3);
}

/**
 * is_digit - Verifies if a character is a digit
 * @c: Character to evaluated
 *
 * Return: 1 if c is a digit, 0 otherwise
 */
int is_digit(char c)
{
	if (c >= '0' && c <= '9')
		return (1);

	return (0);
}

/**
 * convert_size_num - Casts the num to the specified size
 * @num: Num to be casted.
 * @size: Number indicating  type to be casted.
 *
 * Return: Casted value of number
 */
long int convert_size_num(long int num, int size)
{
	if (size == S_LONG)
		return (num);
	else if (size == S_SHORT)
		return ((short)num);

	return ((int)num);
}

/**
 * convert_size_unsgnde - Casts a num to the specified size
 * @num: Num to be casted
 * @size: Num indicating the type to be casted
 *
 * Return: Casted value of number
 */
long int convert_size_unsgnde(unsigned long int num, int size)
{
	if (size == S_LONG)
		return (num);
	else if (size == S_SHORT)
		return ((unsigned short)num);

	return ((unsigned int)num);
}

